...
const FALL_SPEED = 9;
const MAX_TILT = 0.22;       // これ以上傾いたら崩れる
const TILT_SENSITIVITY = 0.005; // 重心ズレ1pxあたりの傾き（ラジアン）
...
export default function RayStack({
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      ctx.fillRect(0, PLATE_Y + 30, CW, 2000);
      ctx.fillStyle = "#0284c7";
      ctx.fillRect(0, CH - 25, CW, 25);

      // タワー全体を傾けて描画（ワールド座標）
      ctx.save();
      ctx.translate(CW / 2, PLATE_Y);
    ...
    const gameLoop = () => {
      ...
      // カメラ追従（スタックトップが画面上部100pxに来るようスクロール）
      const stackTopWorldY = PLATE_Y - s.stack.length * LAYER_H;
      const targetCameraY = Math.min(0, stackTopWorldY - 100);
      s.cameraY += (targetCameraY - s.cameraY) * 0.08;
      ...
      if (s.falling) {
        s.fallY += FALL_SPEED;
        // 着地目標Y（画面座標 = ワールドY - cameraY）
        const landingY = PLATE_Y - (s.stack.length + 1) * LAYER_H - s.cameraY;
  ...
  useEffect(() => {
    ...
    const collapseCameraY = gs.current.cameraY;
    ...
    const loop = () => {
      ...
      // カメラを適用してワールド要素を描画
      ctx.save();
      ctx.translate(0, -collapseCameraY);
      ...
      const allGone = layers.every(l => l.y - collapseCameraY > CH + 60);
      if (frame > 90 || allGone) {
        setPhase("gameover");
        return;
      }
